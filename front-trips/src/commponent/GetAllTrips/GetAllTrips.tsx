...
//   const [data, setData] = useAtom(tripDataAtom);
...
//     const handelDelete = async (deleteTripId: number) => {
//       console.log(typeof(deleteTripId));
//       try {
//         const deleteTrip = await trpc.deleteById.mutate(deleteTripId);
//         console.log(typeof(deleteTripId));
        
//         console.log("delete trip successful!", deleteTrip);
//         setData((newData) => newData.filter((trip) => trip.id !== deleteTripId));
//       } catch (error) {
...
//             fill="none" viewBox="0 0 24 24" stroke="currentColor" onClick={(event) => { handelPreventDeflate(event);  handelDelete(item.id) }}>
...
import useDeleteTrip from "./DeleteTripGlobal";

const GetAllTrips: React.FC = () => {
  const setData = useSetAtom(tripDataAtom);
  const [loading] = useAtom(loadingAtom)
  const {dataAllTrips, getAllTripsGlobal} = useGetAllTrips()
  const { setDataAllTrips, deleteTripGlobal } = useDeleteTrip() 
